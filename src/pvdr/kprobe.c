...
static int probe_teardown_events(kprobe_t *kp)
{
	...
	for (i = 0; i < kp->efds.len; i++) {
		/* err = ioctl(kp->efds.fds[i], PERF_EVENT_IOC_DISABLE, 0); */
		close(kp->efds.fds[i]);
	}
...
static int kprobe_unload(kprobe_t *kp, const char *probestring)
{
	char *func;
...
static int kprobe_destroy(kprobe_t *kp, const char *pattern)
{
	...
	/*
	 * we need to unload after closing all event fds relating to probe,
	 * otherwise we cannot remove the event from
	 * /sys/kernel/debug/tracing/kprobe_events.
	 */
	err2 = kprobe_unload(kp, pattern);
